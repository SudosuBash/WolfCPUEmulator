...
void eret(PWOLF_CPU_ECALL_CONTROLLER* ctrl) {
    WOLF_CPU* cpu = get_parent_struct(ctrl, WOLF_CPU, ecall_controller);
    cpu->spe_regs.bcr |= (GET_DATA_BIT_VALUE(cpu->ecall_regs.mprevmode,KU_STORE_BITS) << (BCR_KERN_MODE_MASK)); //恢复特权级
    cpu->spe_regs.bcr &= 0b11111111 ^ (GET_DATA_BIT_VALUE(cpu->ecall_regs.mprevmode,IRQ_STORE_BITS) << (BCR_IRQ_DISALLOW_MASK_BIT)); //恢复中断使能
